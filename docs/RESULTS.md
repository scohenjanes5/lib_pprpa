...
## Stage 14: the hcore derivative in density space -- job 27160009, allocation 27259554
...
**That reduction is gpu4pyscf's own.**  `multigrid.eval_vpplocG_SI_gradient`
(`eval_nucG_SI_gradient` all-electron) is exactly the expression above, and
`krhf.grad_elec` already uses it on its `multigrid_v2` branch -- it is the
default `KNumInt` branch that falls through to the per-atom loop.  So the
module is only wiring: rho_T(G) from `KNumInt.get_rho`, that reduction, and
`contract_h1e_dm(hermi=0)` for the AO-derivative half.  109 lines, 38 of them
executable.  A first version re-derived the reduction with its own structure
factors, atom batching and device-group grid split (340 lines); its
measurements are kept below because they are a same-card comparison.

Correctness, five independent checks:

| check | agreement |
|---|---|
| pyscf's *CPU* `hcore_generator`, distorted diamond, symmetric T | 1.8e-15 rel. |
| same, non-symmetric T | 8.5e-16 rel. |
| gpu4pyscf `hcore_generator`, NV63 ke=600 (nao 819, mesh 151^3) | 6.33e-15 rel. |
| same, NV216 ke=300 (nao 2795, mesh 159^3), 8 of 215 atoms sampled | 9.28e-14 rel. |
| full `grad_elec`, default vs `PPRPA_HCORE=dense` (C2 diamond) | 5.15e-15 rel. |
...
| cell | per-atom generator | density space | breakdown | speedup |
|---|---|---|---|---|
| NV63, ke=600 | 52.7 s (0.84 s/atom) | **6.0 s** | rho 0.9 / G-space 0.1 / AO-deriv 5.0 | 8.8x |
| NV216, ke=300 | 6.10 s/atom -> 1311 s = **21.8 min** | **17.8 s** | rho 2.8 / G-space 0.1 / AO-deriv 14.9 | **74x** |

The NV216 per-atom side is sampled over 8 atoms and extrapolated; 21.8 min
against the 24 min recorded for one GPU in stage 11 is the cross-check that the
sample is not flattering itself.

What is left is the AO-derivative term, `krhf.get_hcore` -- one array, not
per-atom work.  It is 84% of the remaining 17.8 s, but 15 s inside a ~28 min
force is not worth chasing: splitting its grid pass over the device group buys
~7 s for ~70 lines, so the first version's split was dropped.

The same measurements on an RTX PRO 6000 Blackwell (job 27160009, fp64 ~1/64 of
its fp32) read 786.1 s -> 21.8 s at NV63 and 189.6 s at NV216 (job 27225292).
**The ratio does not transfer between cards**, contrary to what an earlier
revision of this section claimed: 36x there against 8.8x on a B200 at the same
cell.  Between the two cards the per-atom path slows by 14.9x and the density
path by only 3.6x, because the per-atom path is an fp64-GEMM wall while the
density path is dominated by one AO grid pass.  Only the B200 figures are
comparable with the rest of this file.

Tests: `tests/test_gpu_hcore_force.py` (6) -- pseudo and all-electron branches,
a non-symmetric density (what `hermi=0` buys), the device group being inert,
the `PPRPA_HCORE=dense` switch, and the end-to-end `grad_elec` dispatch.
`tests/test_scf_chk.py` (8, CPU only) covers the SCF checkpoint of section 16.
89 tests collected in the suite; all pass from the shared checkout.
